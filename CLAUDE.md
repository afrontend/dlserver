...
npm run mcpsse               # Start MCP server (SSE transport)
...
All search operations use the `dongnelibrary` package's `search()` method, wrapped in Promises with async/await. Error responses return `{message: err.msg}` or `{message: 'Server Error'}`.
...
- Backend uses Promises with async/await; frontend uses callback patterns
